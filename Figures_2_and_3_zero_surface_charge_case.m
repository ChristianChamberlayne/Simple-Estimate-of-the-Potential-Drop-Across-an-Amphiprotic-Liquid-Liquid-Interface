...
if (true)
    ...
    if nm_axis
        if(~pure_liquids)
            ...
            semilogy(-graphx_conc*1e9,ion_all_pos,'LineWidth',2)
            hold on
            semilogy(-graphx_conc*1e9,ion_all_neg,'LineWidth',2)
        end

        semilogy(-x*1e9,ion_H,'LineWidth',2)
        hold on
        semilogy(-x*1e9,ion_OH,'LineWidth',2)
        semilogy(x*1e9,ion_ROH2,'LineWidth',2)
        semilogy(x*1e9,ion_RO,'LineWidth',2)
    else
        if(~pure_liquids)
            ion_all_pos_H2O = water_ionic_strength * exp(-phi_H2O/Vt); 
            ion_all_neg_H2O = water_ionic_strength * exp(phi_H2O/Vt);
...
if nm_axis
    yyaxis left
    plot(graphx*1e9,graphphi,'LineWidth',2,'DisplayName',"\phi(x) (mV)")
    yyaxis right
    plot(graphxE*1e9,graphE,'LineWidth',2,'DisplayName',"E(x) (V/m)")
    hold on
else
    yyaxis left
